...
Download the software [BEAST2 v2.6 or above](https://www.beast2.org), [Tracer v1.7](https://beast.community/tracer) and [FigTree](https://beast.community/figtree) if you haven't done so already.
...
This is currently set to 100, in the box that says `initial = [100.0]`. Change it to something like 40, so it doesn't violate fall outside the minimum and maximum values.
...
Note there's a bug in BEAUti v2.6 that makes it tricky to set up different partitions, so you can use a precooked xml for this purpose. You can download this [here](bears_morpho_cytb.xml) and get it running in BEAST.
...
#TODO: double check all the figs associated with this tutorial
