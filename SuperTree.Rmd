...
## 4. Skyline FBD 
We can relax the assumption of constant speciation, extinction, and fossil sampling in an FBD analysis by using a skyline model. This model allows rates to vary in a piece-wise manner across different geological intervals. Essentially, creating time-bins as in common in many paleontological methods. To do this you need to define your timeline and create a vector of your rates for each bin. 

We can edit the existing FBD.rev file as as follows (copy the FBD.rev file and rename).

Upate the name of the analyis 
...
You can remove how we defined speciation, extintion and fossil sampling from this new skyline. We can then loop through the different time bins to estimate rates for each of them. 
...
>Nothing else in the script needs to be changed for this analysis. Here we have all three parameters varying however we could choose to fix on for example fossil sampling, and then just allows speciation and extinction to vary though time.
