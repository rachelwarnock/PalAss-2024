...
In this exercise we'll estimate a tree of terror birds ([*Brontornis*](https://de.wikipedia.org/wiki/Brontornis)) using morphological character data. The matrix is from [Agnolin et al.  2007](http://revista.macn.gob.ar/ojs/index.php/RevMus/article/view/349/333) and consists of 12 taxa and 51 characters $-$ you can download this [here](data/Agnolin.nex).  
...
Can someone with experience on windows add a blurb here?

## The Rev langauge

RevBayes uses its own language, which has a lot in common with the R programming language. 
One of the main differences is that it includes an explicit set of commands and operators for setting up hierarchical or graphical models. You can learn more about and explore the Rev language [here]().

Briefly, different types of operators are used to explicitly define different types of variables:

- `<-`: [constant variables](exercise-03.html#constant) - variables that are fixed throughout your analysis, e.g., number of taxa
- `~`: [stochastic variables](exercise-03.html#stochastic)- variables that are described by or drawn from some underlying distribution; these are estimated during your analysis, e.g., the tree topology or branch lengths
- `:=`: [deterministic variables](exercise-03.html#deterministic) - variables that can be transformed based on the values of other variables, e.g., total tree length is the sum of all branch lengths
- `=`: used to define variables that are not part of the model, e.g., the vector of MCMC moves

We'll see examples of all of these in setting up your analysis. 
...
```{r data, eval=FALSE}
data <- readDiscreteCharacterData("data/Agnolin.nex")
data
```
...
We'll define a **Q matrix** for X states (A, T, G, C) using the `fnJC` function. Under this model rates of change between states and state frequencies (the proportion of each character state) are assumed to be equal. Later we'll see what happens when we relax this assumption.
...
Let's increase the chain length to 10000 and rerun the MCMC. This might take up to XXXX minutes $-$. #TODO

You're already getting a flavour for how long MCMC analyses can take. #TODO It's very common to have to run empirical analyses for millions+ generations and it can take several days for a single analysis.
...
> Are there any differences between the summary trees?  TODO
