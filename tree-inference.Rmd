...
In this exercise we'll estimate a tree of hyaenodonts ([Hyaenodonta](https://en.wikipedia.org/wiki/Hyaenodonta)) using morphological character data. The matrix is from [Egi et al.  2007](http://revista.macn.gob.ar/ojs/index.php/RevMus/article/view/349/333) and consists of 12 taxa and 65 characters $-$ you can download this [here](data/Egi.nex).  
...
- `<-`: [constant variables](exercise-Rev.html#constant) - variables that are fixed throughout your analysis, e.g., number of taxa
- `~`: [stochastic variables](exercise-Rev.html#stochastic)- variables that are described by or drawn from some underlying distribution; these are estimated during your analysis, e.g., the tree topology or branch lengths
- `:=`: [deterministic variables](exercise-Rev.html#deterministic) - variables that can be transformed based on the values of other variables, e.g., total tree length is the sum of all branch lengths
...
You can open this tree in FigTree. Root the tree along the branch leading to `"missing_ORIG_Procerberus_and_Cimolestes` $-$ this is our outgroup, the flying lemur.
